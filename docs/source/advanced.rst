...
4. **probabilities_tables_generator.py** uses the previously generate table of cells to infer the probabilities for the current subset of cells. The probabilities estimate 1) the probability for a gene to be expressed in any cell; 2) the probability for a gene to be expressed in any cell type reported in the annotation table;
